    ...
    <details className="group border-t border-lab-border bg-lab-bg/45">
      <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-2 px-3 text-[11px] font-semibold text-lab-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-cyan">
        <span className="flex items-center gap-2">
          <ServerIcon className={`h-3.5 w-3.5 ${loading ? "animate-pulse text-accent-amber" : error ? "text-accent-red" : "text-accent-green"}`} />
          Runtime inventory
        </span>
      ...
      <div className="border-t border-lab-border px-3 py-3">
        ...
        {!loading && error && (
          <div role="alert">
            <p className="text-[10px] leading-4 text-red-100/80">{error}</p>
            <Button size="sm" variant="secondary" onClick={onRetry} className="mt-2"><RefreshIcon className="h-3.5 w-3.5" /> Retry</Button>
          </div>
        )}
        {!loading && engines && (
          <>
            ...
            <ul className="divide-y divide-lab-border/75" aria-label="Backend simulation engines">
              {engines.engines.map((engine) => (
                <li key={engine.id} className="py-2">
                  ...
                  {!engine.available && engine.unavailable_reason && <p className="mt-1 text-[9px] leading-4 text-red-100/65">{engine.unavailable_reason}</p>}
                </li>
              ))}
            </ul>
